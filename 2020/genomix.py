...
with open("sample3.json", "r", encoding="utf8") as fin:
...
def build_fcgr(seq, k):
    ...
    newseq = replaceMultiple(seq,
                             ["B", "D", "E", "F", "H", "I", "J", "K", "L", "M", "N", "O", "P", "Q", "R", "S", "U", "V",
                              "W", "X", "Y", "Z"], "")
...
for gr in groups:
    ...
    for ind, acc_num in enumerate(accession_numbers):
        ...
        with open(os.path.join(root_dir, "2020", "data_repo", "{}.fasta".format(acc_num)), "r", encoding="utf8") as fin:
             cur_fcgr = build_fcgr(parse_seq_from_fasta(fin.read()), fcgr_resolution)

        all_fcgrs.append(cur_fcgr)

print(len(all_fcgrs))
print(all_fcgrs[0])


def progress(i,j, total):
    s = j-i
    if i >=1:
        for k in range(i,0,-1):
            s += total - k
    return 2*s / total*(total-1)
...
for i in range(len(all_fcgrs)):
    for j in range(i + 1, len(all_fcgrs)):
        ...
        print(i, j, numerator, denominator, val, progress(i,j,len(all_fcgrs)))
        dist_matrix[i][j] = val
        dist_matrix[j][i] = val
...
delta = dist_matrix.astype(float)
(dim, dim) = delta.shape

deltasq = delta**2
deltatotals = np.sum(deltasq, axis=0)/dim
sumOfDelta = np.sum(deltatotals)/dim
...
# eigValues, eigVectors = scipy.linalg.eigh(dist_matrix)
eigValues, eigVectors = scipy.linalg.eigh(bMatr)
...
json_mapfile = {
    ...
    "groups": dataset["groups"],
    "points": scaledPoints,
    "all_sequences_info": [{"accession_number": "1", "header": "2", "sequence_length": "3", "taxa": 4} for x in range(len(all_fcgrs))],
}
